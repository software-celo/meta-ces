...
# do not mount inactive rauc rootfs partitions
if [ -f /usr/bin/rauc ];then

	DETECTROOT=""

	for option in `cat /proc/cmdline`;do
		if [ $(echo $option | cut -d'=' -f1) == "root" ];then
			DETECTROOT=`echo $option | cut -d'=' -f2`
		fi
	done

	if [ -n $DETECTROOT ];then

		if [ "$DETECTROOT" == "/dev/mmcblk1p1" ];then
			UNUSED_ROOT=/dev/mmcblk1p2
		elif [ "$DETECTROOT" == "/dev/mmcblk1p2" ];then
			UNUSED_ROOT=/dev/mmcblk1p1
		else
			UNUSED_ROOT=""
		fi

		if  [ "$UNUSED_ROOT" == "${DEVNAME}" ];then
			logger "udev/mount.sh [$DEVNAME] is rauc blacklisted, ignoring"
			exit 0
		fi
	fi
fi
